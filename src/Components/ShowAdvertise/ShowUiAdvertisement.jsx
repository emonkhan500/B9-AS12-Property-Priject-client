...
const ShowUiAdvertisement = ({property}) => {
    ...
    return (
        <div >
         ...
         className="rounded-md overflow  shadow-md sm:w-full dark:bg-gray-50 dark:text-gray-800 bg-gradient-to-r from-[#3B9DF8]/30 to-[#00C6FF]/30 backdrop-blur ">
	...
	<div className='flex justify-between px-1 lg:px-4 mt-4 '>
        <h1 className='text-lg md:text-xl  font-semibold'> <span className='text-[#3B9DF8] text-base mr-1'>Title:</span>  {propertyName}</h1>
        <h1 className='flex items-center justify-center gap-1 text-lg md:text-xl font-semibold' ><IoLocationSharp />{location}</h1>
    </div>
    <div className='flex gap-1 md:gap-3 lg:flex-row justify-between px-1 lg:px-4'>
   ...
   <Link to={`/details/${_id}`}> <button className=' px-36 py-3 shadow-md w-full  flex justify-center items-center bg-gradient-to-r from-[#3B9DF8] to-[#3BE8C4]  text-white gap-2'><TbListDetails /> Details</button></Link>
</div> 
        </div>
    );
};
